...
cantril_mini <- cantril %>%
  ...
  mutate(`Absolute change` = round(`2023` - `2011`, 2),
         Relative_change = round((`2023` - `2011`)/`2011`, 2),
         Relative_change1 = scales::percent(Relative_change, accuracy = 0.01,
                                           suffix = "%"))
reactable(cantril_mini)
...
#          Relative_change1 <- scales::percent(Relative_change, accuracy = 0.01,
#                                           suffix = "%")
#          Relative_change2 <- Relative_change1(Relative_change)
# reactable(cantril_mini)
```
...
cantril_filter <- cantril %>%
  ...
  labs(x = "Year",
      y = "Cantril Ladder Score",
      caption = "Data Source: World Hapiness Report")+
  theme_minimal()
ggplotly(cantril_filter)
...
```{r}
performance_top_10 <- cantril %>%
  group_by(Country) %>%
  filter(Year == 2023) %>%
  arrange(`Year`,`Cantril ladder score`) %>%
  head(n = 10)
reactable(performance_top_10)
```

**Bottom 10 countries in 2023**
```{r}
performance_bottom_10 <- cantril %>%
  group_by(Country) %>%
  filter(Year == 2023) %>%
  arrange(desc(`Cantril ladder score`)) %>%
  head(n = 10)
reactable(performance_bottom_10)
#performance_bottom_10
```

```{r}
merge_top_bottom <- bind_rows(performance_top_10, performance_bottom_10)
merge_top_bottom
```

```{r}
  top_bottom_chart <- merge_top_bottom %>%
  ggplot(mapping = aes(x = `Cantril ladder score`,
                       y = Country))+
  geom_bar()+
  #scale_y_continuous(limits = c(0,10)) +
  #labs(x = "Year",
